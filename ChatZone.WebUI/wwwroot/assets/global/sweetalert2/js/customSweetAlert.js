...
function getCookie(cname) {
    // Append an equal sign to the cookie name to prepare for value extraction
    let name = cname + "=";
    // Decode the entire cookie string to handle encoded characters
    let decodedCookie = decodeURIComponent(document.cookie);
    // Split the cookie string into individual name=value pairs
    let ca = decodedCookie.split(';');
    // Iterate over each name=value pair
    for (let i = 0; i < ca.length; i++) {
        // Get one cookie name=value pair
        let c = ca[i];
        // Remove leading whitespace from the cookie name=value pair
        while (c.charAt(0) == ' ') {
            c = c.substring(1);
        }
        // Check if the current name=value string starts with the cookie name we're looking for
        if (c.indexOf(name) == 0) {
            // If found, return the value part of the name=value pair, decoded
...
function deleteCookie(cookieName) {
    // Set the cookie with the specified name to have an empty value
    // and set its expiration date to a past date (Thu, 01 Jan 1970 00:00:00 GMT)
